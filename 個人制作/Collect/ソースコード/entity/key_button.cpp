...
constexpr Texture texture_keyboard = Texture::T_Enter_key;
constexpr Texture texture_xinput = Texture::T_Red_button;

KeyButton::KeyButton(Scene* parent) : Entity(parent)
{
	this->tag_ = "key button";

	auto model = ModelImport::UIData(Window::GetWidth<float>() / 2 - 100, Window::GetHeight<float>() / 2 + 100, 700, 110, 0, 0, 1, 1);
...
void KeyButton::Draw(RenderingPass pass)
{
