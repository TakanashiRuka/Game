...
constexpr Texture texture = Texture::T_Number_digital;

Time::Time(Scene* parent, double time) : Entity(parent), time_(time)
{
	this->tag_ = "time";

	auto model = ModelImport::Number(0, 0, 46 * 2, 75, 0, 2, true);
	...
	XAudio2::AddSourceVoice("forest", SoundType::BGM);
}
...
void Time::Update()
{
	if (!this->start_ && (KeyBoard::Trigger(VK_RETURN) || XInput::Trigger(XINPUT_GAMEPAD_B)))
	{
		XAudio2::StartSourceVoice("forest");
		this->start_ = true;
	}

	if (this->start_ == true && this->time_ > 1)
		this->time_ -= Game::GetInstance()->GetRunningTime();
	else if (this->time_ < 1)
	{
		this->time_ = 1;
		this->parent_->AddEntity<TimeUp>();
	}
	auto model = ModelImport::Number(Window::GetWidth<float>() / 2 + 65, 0, 48 * 2, 80, static_cast<unsigned int>(this->time_), 2, true);
	this->data_->UpdateVertexBuffers(this->tag_, model);
}

void Time::Draw(RenderingPass pass)
{
