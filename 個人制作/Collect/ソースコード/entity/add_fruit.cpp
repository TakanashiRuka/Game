...
constexpr Texture texture = Texture::T_Add_fruit;

AddFruit::AddFruit(Scene* parent) : Entity(parent)
{
	this->tag_ = "add fruit";

	float width = 222;
	float height = 41;
	auto model = ModelImport::UIData(Window::GetWidth<float>() / 2 - width / 2, Window::GetHeight<float>() / 2 - height / 2 + 36 * 3, width, height, 0, 0, 1, 1);
...
void AddFruit::Draw(RenderingPass pass)
{
