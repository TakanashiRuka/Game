...
HighScore::HighScore(Scene* parent) : Entity(parent)
{
	this->tag_ = "high score";
	...
	else
	{
		...
		this->high_score_ = 500;
		output_file.write(reinterpret_cast<char*>(&this->high_score_), sizeof(this->high_score_));
	}
	auto model = ModelImport::Number(Window::GetWidth<float>() - 22 * 8, 0, 22 * 8, 32, this->high_score_, 8, false);
...
void HighScore::Draw(RenderingPass pass)
{
