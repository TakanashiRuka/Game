...
constexpr Texture texture = Texture::T_Speed_up;

SpeedUp::SpeedUp(Scene* parent) : Entity(parent)
{
	this->tag_ = "speed up";

	float width = 194;
	float height = 36;
	auto model = ModelImport::UIData(Window::GetWidth<float>() / 2 - width / 2, Window::GetHeight<float>() / 2 - height / 2 + height, width, height, 0, 0, 1, 1);
...
void SpeedUp::Draw(RenderingPass pass)
{
